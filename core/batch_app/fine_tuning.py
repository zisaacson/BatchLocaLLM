...
class ExportDatasetRequest(BaseModel):
    ...
    philosopher: str = Field(..., description="User email")
    domain: str = Field(..., description="Organization domain")
    conquest_type: str | None = Field(None, description="Optional conquest type filter")
...
async def export_dataset(request: ExportDatasetRequest):
    ...
    Fetches gold star conquests from Aristotle database and exports
    to specified format (ChatML, Alpaca, or OpenAI).
    """
    try:
...
async def create_training_job(request: CreateTrainingJobRequest):
    ...
    gold star conquests for the user.
    """
    try:
        db = next(get_db())
        ...
        training_job = TrainingJob(
            ...
            dataset_path=str(dataset_path),
            sample_count=sample_count,
            conquest_types=[request.conquest_type] if request.conquest_type else None,
            status="running",
            progress=0,
